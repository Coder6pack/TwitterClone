...
const { notEmpty: nameNotEmpty, ...modifiedNameSchema } = nameSchema
...
const { notEmpty: dateOfBirthNotEmpty, ...modifiedDateOfBirthSchema } = dateOfBirthSchema
      ...
      old_password: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            ...
            const { password } = user
            const checkPassword = hashPassword(value) === password
            if (!checkPassword) {
              throw new ErrorWithStatus({
